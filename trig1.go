...
// Trig1 outputs 1 for the specified duration when a nonpositive to positive transition occurs
...
func (t Trig1) Rate(rate int8) Input {
	CheckRate(rate)
	(&t).defaults()
	return NewUgenInput("Trig1", rate, 0, 1, t.In, t.Dur)
}
